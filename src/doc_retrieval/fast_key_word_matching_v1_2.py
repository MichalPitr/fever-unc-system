...
def sample_answer_with_priority(d_list, tokenizer, keyword_p, top_k=5, tokenized=False):
    count = 0
    num_of_select = Counter()
    print("Build results file...")
    for item in tqdm(d_list):
    ...
    for item in tqdm(d_list):
        ...
        for i, (doc_id, priority) in enumerate(finded_keys):
            if priority == 1.0:
                ...
                doc_id_tokens = (common.doc_id_to_tokenized_text(doc_id)).split(' ')
...
def check_inside_paretheses_overlap(doc_id_tokens, doc_id_lemmas, claim_tokens, claim_lemmas):
    ...
    if len(set.intersection(set(p_did_tokens), set(claim_tokens))) > 0:
        return 3.0
    elif len(set.intersection(set(p_did_lemmas), set(claim_lemmas))) > 0:
        return 2.0
    else:
        return 1.0
